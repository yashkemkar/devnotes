...
Border box means that whenever we have an element or a container and we give it a border/outline. We want it to be contained within the width of the element and not outside. Throw this in at the start of every style sheet. You can import a font family in this part of the code at the very start and apply to whole document.
...
#### Display:grid
This is the easiest way to center an item inside a container, along with a place items function.
...
### Hover
```.icon-link:hover {....}```
Throwing a hover after a colon will set a condition allowing this style to only be executed when the condition is met. For example, when hovering over, background is grey.

### Children of Flex Box
```
.footer-grid {
        display: flex;
        flex-direction: row;
...
### Mediaquery
With the advent of smartphones and tablets, there are so many different screen sized - we can't know which one someone is using. So the website must be programmed to be able to handle all sizes and active "responsively". The structure of a website will be exactly the same, however the visuals/behaviour
...
### Selection Tag
A selection tag can be used to target every instance of any HTML tag within the document when it is selected by the user (ie, highlighting text). It can be used to stylise en masse.
```
...
### Pseudo-element
You can put elements into the website directly from CSS, without coding it into HTML.
```
...
```
This is known as a pseudo-element. This is basically a fake element that doesn’t get coded into the html, but gets added by the CSS if you want a particular look or effect. 

### After 
The ::after in the above example ties it to the parent element and tells it to position after/before etc. The content: ‘’; variable is in there to tell us that there is no text, but can put that in.

### Sticky Header - Essential
